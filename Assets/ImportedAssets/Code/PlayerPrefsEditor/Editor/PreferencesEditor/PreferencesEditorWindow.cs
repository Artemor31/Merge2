...
#region ErrorValues
        ...
        private SerializedProperty[] userDefListCache = Array.Empty<SerializedProperty>();
        ...
        private void InitReorderedList()
        {
            if (prefEntryHolder == null)
            {
                ...
                prefEntryHolder = tmp.Length > 0 ? tmp[0] : CreateInstance<PreferenceEntryHolder>();
            }
            ...
            userDefList.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
            {
                ...
                SerializedProperty value = (PreferenceEntry.PrefTypes)type.enumValueIndex switch
                {
                    PreferenceEntry.PrefTypes.Float => element.FindPropertyRelative("m_floatValue"),
                    PreferenceEntry.PrefTypes.Int => element.FindPropertyRelative("m_intValue"),
                    PreferenceEntry.PrefTypes.String => element.FindPropertyRelative("m_strValue"),
                    _ => element.FindPropertyRelative("This should never happen")
                };
                ...
                Rect rect1 = new Rect(rect.x + spliterPos + 62, rect.y, rect.width - spliterPos - 60, EditorGUIUtility.singleLineHeight);
                switch ((PreferenceEntry.PrefTypes)type.enumValueIndex)
                {
                    case PreferenceEntry.PrefTypes.Float:
                        EditorGUI.DelayedFloatField(rect1, value, GUIContent.none);
                        break;
                    case PreferenceEntry.PrefTypes.Int:
                        EditorGUI.DelayedIntField(rect1, value, GUIContent.none);
                        break;
                    case PreferenceEntry.PrefTypes.String:
                        EditorGUI.DelayedTextField(rect1, value, GUIContent.none);
                        break;
                }
            ...
            userDefList.onAddDropdownCallback = (_, _) =>
            {
        ...
        private void CreatePrefEntries(string[] keySource, ref List<PreferenceEntry> listDest)
        {
            ...
            {
                case PreferencesEntrySortOrder.Asscending:
                    listDest.Sort((x, y) => x.m_key.CompareTo(y.m_key));
                    break;
                case PreferencesEntrySortOrder.Descending:
                    listDest.Sort((x, y) => y.m_key.CompareTo(x.m_key));
                    break;
            }
        }
        ...
        private void LoadKeys(out string[] userDef, out string[] unityDef, bool reloadKeys)
        {
            ...
            Dictionary<bool, List<string>> groups = keys.GroupBy(key => key.StartsWith("unity.") ||
                                                                        key.StartsWith("UnityGraphicsQuality"))
                                                        .ToDictionary(g => g.Key, g => g.ToList());

            unityDef = groups.TryGetValue(true, out List<string> group1) ? group1.ToArray() : Array.Empty<string>();
            userDef = groups.TryGetValue(false, out List<string> group) ? group.ToArray() : Array.Empty<string>();
        }
        ...
        private SerializedProperty GetUserDefListElementAtIndex(int index, SerializedProperty ListProperty)
        {
            ...
            return userDefListCache[index] ?? (userDefListCache[index] = ListProperty.GetArrayElementAtIndex(index));
        }
